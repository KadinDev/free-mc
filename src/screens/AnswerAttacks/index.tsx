...
export function AnswerAttacks(){
  const [answerAtual, setAnswerAtual] = useState('');
  ...
  useEffect(() => {
    if(isAutomatic){
      const interval = setInterval(() => {
        ...
        setAnswerAtual(palavraAleatoria);
      }, 30000);
  ...
  return (
    <Container >
      ...
      <TitlePage> Responda os ataques </TitlePage>
      <TotalPalavras> {answerAttacks.length} ataques </TotalPalavras>
      <Separator/>
  
      {isAutomatic && 
        <DivCountdown>
          <TitleCountdown> proxima ataque em </TitleCountdown>
          <Countdown> {countdown} </Countdown> 
        </DivCountdown>
      }

      <DivPalavraAleatoria>
        <PalavraAleatoria> {answerAtual} </PalavraAleatoria>
      </DivPalavraAleatoria>
