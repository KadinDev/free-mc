...
import { themes } from '../../utils/ThemesRandom';
...
export function ThemesTraining(){
  const [themesAtual, setThemesAtual] = useState('');
  ...
  useEffect(() => {
    if(isAutomatic){
      const interval = setInterval(() => {
        const indiceAleatorio = Math.floor(Math.random() * themes.length);
        const palavraAleatoria = themes[indiceAleatorio];
        setThemesAtual(palavraAleatoria);
      }, 30000);
  ...
  return (
    <Container >
      ...
      <TitlePage> Treinar Com Temas </TitlePage>
      <TotalPalavras> {themes.length} Temas </TotalPalavras>
      <Separator/>
  
      {isAutomatic && 
        <DivCountdown>
          <TitleCountdown> proxima tema em </TitleCountdown>
          <Countdown> {countdown} </Countdown> 
        </DivCountdown>
      }

      <DivPalavraAleatoria>
        <PalavraAleatoria> {themesAtual} </PalavraAleatoria>
      </DivPalavraAleatoria>
